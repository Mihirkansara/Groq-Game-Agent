...
async def number_game_node(state: WorkflowState):
    ...
    try:
        ...
        ai_text = result['ai'] if isinstance(result,dict) else result.ai
        game_status = result['status'] if isinstance(result,dict) else result.status
    ...
    updated_state["answer"] = ai_text # Store the LLM's answer or quetions
    updated_state["game_status"] = game_status # Store game status
    updated_state["chat_history"] = chat_history + [{"role": "ai","content":ai_text}]
